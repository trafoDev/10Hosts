...
docker exec cli peer chaincode query --channelID ${CHANNEL_ID} --name ${CC_NAME} -c '{"Args":["queryCar","CAR0"]}'
# peer0.org2
...
docker exec -e CORE_PEER_ADDRESS=peer0.org2.hlftest.com:8051 \
            ...
            cli peer chaincode query --channelID ${CHANNEL_ID} --name ${CC_NAME} -c '{"Args":["queryCar","CAR0"]}'
# peer0.org0
